...
/*-*
 * Provides ad-hoc polymorphism on a specific group of objects that
 * have the same interface (duck-typed).
 */
package struct Any(Ducks...)
{
    /**
     ...
     * Throws:
     * $(UL
     *   $(LI $(D Error) if this $(D Any) object is empty)
     * )
     */
    ...
    {
        if (_which == size_t.max || rhs._which == size_t.max)
    ...
    /**
     * The $(D Any) namespace provides various 'meta' methods
     * for operating on this $(D Any) object itself, not a
     * contained object.
     *
     * Example:
--------------------
...
assert(ab.Any.empty);
assert(ab.Any.allows!A);

ab = A();
assert(ab.Any.isActive!A);

A a = ab.Any.instance!A;
--------------------
     */
    template _Any()
    {
        ...
        /**
         * Returns $(D true) if type $(D T) is listed in the type
         * list $(D Types).
         */
        ...
        /**
         * Returns $(D true) if this $(D Any) object contains
         * nothing.
         */
        ...
        {
            return _which != size_t.max && _which == _duckID!T;
        }
         ...
         * Throws:
         * $(UL
         *   $(LI $(D assertion) fails if the $(D Any) object is
    ...
    //----------------------------------------------------------------//
private:
    // The internal symbols are prefixed with an underscore so that
    // opDispatch will not be blocked. @@@
    ...
    /*
     * Returns $(D true) if the type $(D T) is in the set of types
     * $(D Ducks).
     */
     ...
     * $(D T) is not in the set of the types.
     */
        ...
        if (_homogenizes!(T))
    {
        ...
        {
            static if (_duckID!T == _duckID!Duck)
                return *cast(const Duck*) _storage.ptr;
        }
        assert(0);
    }
    ...
    //----------------------------------------------------------------//
private:
    size_t _which = size_t.max;     // ID of the 'active' Duck
    union
    {
        ubyte[_maxSize!(0, Ducks)]                  _storage;
        void*[_maxSize!(0, Ducks) / (void*).sizeof] _mark;
    }
}

// These templates are prefixed by _ because templates are always public
// even if they have the private attribute.
...
unittest
{
    ...
    assert(is(a.Any.Types == TypeTuple!(int, real)));
    assert(a.Any.allows!int);
    assert(a.Any.allows!real);
    assert(!a.Any.allows!string);

    assert(a.Any.empty);

    a = 42;
    assert(!a.Any.empty);
    assert( a.Any.isActive!int);
    assert(!a.Any.isActive!real);
    assert(!a.Any.isActive!string);
    int* i = &(a.Any.instance!int());
    assert(*i == 42);
...
{
    // implicit convertion
    Any!(real, const(char)[]) a;
    a = 42;
    assert(a.Any.isActive!real);
    a = "abc";
    assert(a.Any.isActive!(const(char)[]));
}

unittest
{
    // foreach over input range
    Any!(string, wstring) str;
