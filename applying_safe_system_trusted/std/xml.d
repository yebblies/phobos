...
class Element : Item
{
     ...
     */
    @trusted /* for AssociativeArray.opApply */
    this(const(Tag) tag_)
    {
    ...
    const
    {
        ...
        override string[] pretty(uint indent=2)
        {
            ...
            if (items.length == 1)
            {
                const t = cast(const Text)(items[0]);
                if (t !is null)
                {
...
class XMLException : Exception { @trusted this(string msg) { super(msg); } }
