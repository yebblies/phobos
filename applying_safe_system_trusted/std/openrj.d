...
public:
    ...
    int opApply(scope int delegate(ref Field field)@safe dg)
    {
        int result  =   0;
     ...
     */
    @trusted /* @@@BUG@@@ workaround for bugzilla 4218 */
    int opApply(scope int delegate(in string name, in string value)@safe dg)
    {
        int result  =   0;
...
public:
    ...
    /// ditto
    @safe
    int opApply(scope int delegate(ref Record record)@safe dg)
    {
        int result  =   0;
